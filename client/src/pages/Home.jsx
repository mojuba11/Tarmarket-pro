...
const Home = () => {
  ...
  // Logic for the Live Payout Toast
  useEffect(() => {
  ...
  return (
    <div className="bg-tarmarket-cream min-h-screen font-sans flex flex-col text-tarmarket-umber overflow-x-hidden relative">
      ...
      {/* 1. LIVE PAYOUT TOAST (Floating UI) */}
      ...
      {/* 2. THE BOXED WRAPPER (Desktop Constraint) */}
      <div className="max-w-[1440px] mx-auto w-full bg-white shadow-xl md:shadow-2xl relative">
        ...
        <div className="absolute top-1/2 -right-20 w-96 h-96 bg-tarmarket-umber rounded-full mix-blend-multiply filter blur-3xl opacity-5"></div>
        ...
        <main className="flex-grow relative z-10">
          ...
          <section className="py-16 md:py-32 px-6 text-center max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-center gap-4">
              <button className="bg-tarmarket-umber text-white px-10 py-4 rounded-xl shadow-xl hover:-translate-y-1 hover:shadow-2xl active:scale-95 transition-all font-bold text-lg">
                FOR INFLUENCERS
              </button>
              <button className="bg-white text-tarmarket-umber px-10 py-4 rounded-xl border-2 border-tarmarket-umber hover:bg-tarmarket-tan hover:-translate-y-1 transition-all font-bold text-lg">
                FOR WORKERS
              </button>
            </div>
          </section>
          ...
          <div className="bg-tarmarket-tan/20 py-8 border-y border-tarmarket-tan/50 overflow-hidden flex whitespace-nowrap">
            <div className="flex shrink-0 animate-marquee gap-12 px-6 opacity-40 font-black text-xl tracking-widest uppercase">
              <span>YouTube</span><span>TikTok</span><span>Instagram</span><span>Facebook</span><span>Threads</span><span>Twitter</span><span>YouTube</span><span>TikTok</span>
            </div>
            <div className="flex shrink-0 animate-marquee gap-12 px-6 opacity-40 font-black text-xl tracking-widest uppercase">
              <span>YouTube</span><span>TikTok</span><span>Instagram</span><span>Facebook</span><span>Threads</span><span>Twitter</span><span>YouTube</span><span>TikTok</span>
            </div>
          ...
          <section className="py-16 md:py-24 bg-white border-b border-tarmarket-tan/30">
            <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
              {[
                { label: "Watch Hours Delivered", val: "50k+" },
                { label: "Verified Workers", val: "12k+" },
                { label: "Total Payouts", val: "₦10M+" }
              ].map((stat, i) => (
                <div key={i} className="space-y-2 group">
                  <p className="text-5xl md:text-6xl font-black group-hover:text-tarmarket-clay transition-colors">{stat.val}</p>
                  <p className="text-tarmarket-clay font-bold text-sm tracking-widest uppercase">{stat.label}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="py-20 px-6">
            <div className="max-w-3xl mx-auto bg-tarmarket-cream border border-tarmarket-tan p-8 md:p-12 rounded-[3rem] shadow-sm text-center transition-all hover:shadow-md">
          ...
          <section className="py-20 md:py-32 px-6 text-center bg-tarmarket-umber text-white relative">
            <div className="relative z-10">
              ...
              <button className="bg-tarmarket-tan text-tarmarket-umber px-14 py-5 rounded-full text-xl font-black hover:bg-white hover:scale-110 active:scale-95 transition-all shadow-xl uppercase tracking-widest">
                Get Started Now
              </button>
            </div>
          </section>
